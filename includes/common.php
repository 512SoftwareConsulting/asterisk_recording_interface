...
/**
 * Configure for asterisk realtime (if used)
 */
function configureRealtime() {

  global $ASTERISK_EXTCONFIG_CONF;
  global $realtime;

  if (!is_writable($ASTERISK_EXTCONFIG_CONF)) {
    $_SESSION['ari_error'] = 
      sprintf(_("%s does not exist or is not writable"),$ASTERISK_EXTCONFIG_CONF);
  }
  else {

    $found = 0;
    $lines = file($ASTERISK_EXTCONFIG_CONF);
    foreach ($lines as $key => $line) {

      $line = trim($line);
 
      // parse if already found 'settings' header
      if ($found && substr($line,0,1)!=';') {
        ...
        if ($value!='') {
          ...
          $realtime[$var][dbengine] = $data[0];
          $realtime[$var][dbname] = $data[1];
          $realtime[$var][dbtable] = $data[2];
        }
      }

      // look for 'settings' header
      if (strpos($line,'[settings]')===FALSE) {
        $found = 1;
      }
    }
  }
...
function handler() {
  ...
  // configure for Asterisk Realtime
  configureRealtime();
...
// build asterisk realtime config array
$realtime = array();
